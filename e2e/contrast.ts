 ...
 *    `color-mix()`. Every `.panel` on this page is a `linear-gradient(180deg,
 *    var(--panel), var(--panel-2))`, the body carries two radial washes, the
 *    hero's "why it matters" aside is a `color-mix(in oklab, …)`, and the
 *    primary buttons, the guided-path numerals and the stop badges are all
 *    `linear-gradient(135deg, var(--server), var(--purple))`. axe files text
 *    over any of those under `incomplete` rather than measuring it.
 ...
 *    `opacity:.85`, and the never-combine path's "full secret key buffer" card
 *    is why this helper earns its keep in this repo: it was authored at
 *    `opacity:.6` over `--muted` text, which axe reports as an untouched
 *    #aeb9cf while the compositor actually paints 3.86:1.
 *    Note this lab collapses its animations to 0.01ms under reduced motion,
 *    which preserves end states — including the `trace-pop` keyframe, whose
 *    `from` step is `opacity: .2`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This page has two
 *    such boxes: `.log-panel` (`max-height: 420px; overflow: auto`), which after
 *    a full signing run holds more rows than fit, and `.table-wrap`
 *    (`overflow-x: auto`) around the five-column research landscape table.
 ...
 *    has no SVG *text* at all — its only SVG is the shared header's hamburger
 *    and GitHub marks, both `aria-hidden` and glyph-free — so the sibling walk
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" aside is `color-mix(in oklab, …)` and the shared header's ink is
     * `color-mix(in srgb, …)`, and Chromium reports both to `getComputedStyle`
     * unchanged rather than converting them to sRGB. A
     ...
     * gradient whose worst stop spans its element. It is wrong for every panel
     * on this page: `.panel` is `linear-gradient(180deg, var(--panel),
     * var(--panel-2))` over a box up to 900px tall, and text near the top sits
     * on `--panel` while text near the bottom sits on `--panel-2` — two
     * different surfaces, and in light theme #ffffff versus #eef3fb. So each
     ...
     * `body` carries two `radial-gradient(circle at top …)` washes that tint the
     * top of the page purple and cyan, and every panel above the fold is
     * translucent enough for them to show through.
     */
     ...
     *
     * A driven scan here walks the two open glossaries (fifteen `<dt>`/`<dd>`
     * pairs between them), the six-row research table, the protocol log, the
     * three trace lanes, the three never-combine lanes, the per-attempt
     * rejection trace and four verdict cards, and every one of those nodes
     ...
     * that is `.log-panel` and `.table-wrap`.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has exactly that case, twice: the round-trace
      // glossary and the research-table glossary are both `<details>`, and both
      // are closed at first paint. `checkVisibility()` catches them. The gate
      // opens each one by clicking its `<summary>` — the route a reader has —
      // and scans the open state, rather than setting `.open` from script.
      ...
      // uses it twice: the shared header's `.cl-skip-link` parks at `top:-3rem`
      // and the lab's own `.skip-link` at `top:-48px`, each sliding in only on
      // focus. Measuring the parked copies invents failures for text that is not
      // on screen; the focused renderings are real states and the gate scans
      // both of them explicitly instead.
     ...
     * it, the `aria-hidden` marks this lab carries — the ████████ blocks
     * standing in for each masked key share (paired with an `.sr-only` "value
     * hidden"), the `+` and `= (mod 256)` operators in the share-combine widget,
     * the •/✓/✕ verdict glyphs, the ✓/↻ per-attempt glyphs and the two flight
     * layers the animated chips fly through — would be measured as if each took
     * its element's declared `color`.
     */
     ...
     * silent, and this page builds nearly all of its markup by interpolating
     * into template literals, which is exactly where it happens: interpolating a
